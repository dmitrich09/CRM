...
class ProviderController extends BaseController
{
    ...
    */
    public function actionIndex()
    {
		...
		$q = Provider::find()->andWhere(['account_id' => self::$user->account_id]);
		$q->orderBy('name asc');
		...
		if ($query) {
			$q->andWhere('lower(name) like :q or lower(email) like :q or lower(lawadress) like :q or lower(postadress) like :q or lower(adress) like :q or lower(contact) like :q or lower(phone) like :q', ['q' => $query ]);
		}
    ...
    */
	...
	self::createLog('provider', serialize($model));
    ...
    *     summary="getall provider",
    *    description="getall  provider",
    *     tags={"Clients"},
    *  
    *     @OA\Response(
    *         response=200,
    ...
    */
    public function actionDelete()
	{
		...
		if (!$model) {
			self::error('Provider not found with id: ' . $id);
			return;
		}
		if ($model->deleted_at) {
...
/**
    * @OA\Get(
    *     path="/provider/restore",
    *     summary="Restore provider",
    *    description="restore  provider",
    *     tags={"Provider"},
    *     @OA\Parameter(
    *         description="Provider id to delete",
    *         in="query",
    ...
    */
    public function actionUpdate()
    {
		...
		return self::error('Model deleted ');
	        ...
	        $modelNew = new Provider();
        $modelNew->setAttributes(Yii::$app->request->post());
		$model->name = $modelNew->name; 
		$model->email = $modelNew->email; 
		$model->lawadress = $modelNew->lawadress; 
		$model->postadress = $modelNew->postadress; 
		$model->adress = $modelNew->adress; 
		$model->contact = $modelNew->contact; 
		$model->phone = $modelNew->phone; 
        
		if ($model->save()) {
	        self::updateLog('provider', $id);
            return self::ok($model);
        }
        return self::error($model->getErrors());
    }
    ...
    public function actionDownloadlist()
    {
		...
		return Excel::export([
            ...
            'fileName' => $table,
            'models' => $resarr,
            'columns' => ['id', 'name', 'account_id', 'email', 'lawadress', 'postadress', 'adress', 'contact', 'phone', 'deleted_at', ],
            'headers' => ['id'  => 'id', 'name'  => 'name', 'account_id'  => 'account_id', 'email'  => 'email', 'lawadress'  => 'lawadress', 'postadress'  => 'postadress', 'adress'  => 'adress', 'contact'  => 'contact', 'phone'  => 'phone', 'deleted_at'  => 'deleted_at', ],
        ]);
    }
	...
	private function issetParams(array $array){
		$this->error = [];
		$nameArr = ['id', 'name', 'account_id', 'email', 'lawadress', 'postadress', 'adress', 'contact', 'phone', 'deleted_at', ];
