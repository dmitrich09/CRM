...
class OutcallController extends BaseController
{
    const RES_TRUE = 10;
    const RES_FALSE = 20; 
    const RES_NOONE = 30;
    ...
    *    tags={"Outcall"},
    *    path="/outcall",
    *    summary="list outcalls",
    *    description="find outcall",
    *     @OA\Parameter(
    ...
    *        @OA\JsonContent(
    *            type="array",
    *            @OA\Items(ref="#/components/schemas/Outcall")
    *         )
    *    ),
    ...
    */
    public function actionIndex()
    {
		...
		if($userId) {
		    $q->andWhere(['o.user_id' => $userId]);
	    }
		if($dateFrom) {
		    $q->andWhere('o.startdate >= :dateFrom', ['dateFrom' => $dateFrom]);
	    }
		if($dateTo){
			$q->andWhere('o.startdate <= :dateTo', ['dateTo' => $dateTo]);
		}
	    if($query) {
    ...
    */
    public function actionGetall()
    {
		...
		return self::ok( Outcall::find()->andWhere(['account_id' => self::$user->account_id])->orderBy('id asc')->all() );  
    ...
    *    tags={"Outcall"},
    *    path="/outcall/create",
    *    summary="create  outcall ",
    *    description="create new outcall",
    *    @OA\RequestBody(
    *        @OA\MediaType(
    *            mediaType="application/json",
    *            @OA\Schema(ref="#/components/schemas/Outcall")
    *          )
    *     ),
    *    @OA\Response(
    *        response=200,
    *        description="OK",
    *        @OA\JsonContent(ref="#/components/schemas/Outcall")
    *    ),
    *    security={{"api_key":{"PHPSESSID"}}}
    * )
    */
    public function actionCreate()
    {
		...
		if ($model->enddate) {
			$model->enddate = ($model->enddate ? date("Y-m-d H:i:s", strtotime($model->enddate)) : null);
		}
		if ($model->startdate) {
			$model->startdate = ($model->startdate ? date("Y-m-d H:i:s", strtotime($model->startdate)) : null);
		}       
		if (!$model->save()) {
            return self::error($model->getErrors());
        }
	    self::createLog('outcall', serialize($model));
    ...
    * @OA\Delete(
    *     path="/outcall/delete",
    *     summary="Delete outcall",
    *    description="delete  outcall",
    *     tags={"Outcall"},
    *     @OA\Parameter(
    *         description="Outcall id to delete",
    *         in="query",
    *         name="id",
    *         required=true,
    *         @OA\Schema(
    ...
    */
    public function actionDelete()
	{
		...
		if (!$model) {
			self::error('Outcall not found with id: ' . $id);
			return;
		}
				if ($model->deleted_at) {
...
/**
    * @OA\Get(
    *     path="/outcall/restore",
    *     summary="Restore outcall",
    *    description="restore  outcall",
    *     tags={"Outcall"},
    *     @OA\Parameter(
    *         description="Outcall id to delete",
    *         in="query",
    ...
    *    tags={"Outcall"},
    *    path="/outcall/update",
    *    summary="update  outcall ",
    *    description="update outcall",
    *    @OA\RequestBody(
    *        @OA\MediaType(
    *            mediaType="application/json",
    *            @OA\Schema(ref="#/components/schemas/Outcall")
    ...
    */
    ...
    public function actionDownloadlist()
    {
		...
		return Excel::export([
            ...
            'fileName' => $table,
            'models' => $resarr,
            'columns' => ['id', 'clients_id', 'account_id', 'user_id', 'contactdate', 'comment', 'status', 'showed', 'enddate', 'startdate', 'declinematter_id', 'deleted_at', ],
            'headers' => ['id'  => 'id', 'clients_id'  => 'clients_id', 'account_id'  => 'account_id', 'user_id'  => 'user_id', 'contactdate'  => 'contactdate', 'comment'  => 'comment', 'status'  => 'status', 'showed'  => 'showed', 'enddate'  => 'enddate', 'startdate'  => 'startdate', 'declinematter_id'  => 'declinematter_id', 'deleted_at'  => 'deleted_at', ],
        ]);
    }
	...
	private function issetParams(array $array){
		$this->error = [];
		$nameArr = ['id', 'clients_id', 'account_id', 'user_id', 'contactdate', 'comment', 'status', 'showed', 'enddate', 'startdate', 'declinematter_id', 'deleted_at', ];
