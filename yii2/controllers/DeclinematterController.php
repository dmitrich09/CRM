...
class DeclinematterController extends BaseController
{
    const RES_TRUE = 10;
    const RES_FALSE = 20; 
    const RES_NOONE = 30;
    ...
    *    tags={"Declinematter"},
    *    path="/declinematter",
    *    summary="list declinematters",
    *    description="find declinematter",
    *     @OA\Parameter(
    ...
    *        @OA\JsonContent(
    *            type="array",
    *            @OA\Items(ref="#/components/schemas/Declinematter")
    *         )
    *    ),
    ...
    */
    public function actionIndex()
    {
		...
		$q = Declinematter::find()->andWhere(['account_id' => self::$user->account_id]);
		$q->orderBy('id asc');
    ...
    * @OA\Post(
    *    tags={"Declinematter"},
    *    path="/declinematter/create",
    *    summary="create  declinematter ",
    *    description="create new declinematter",
    *    @OA\RequestBody(
    *        @OA\MediaType(
    *            mediaType="application/json",
    *            @OA\Schema(ref="#/components/schemas/Declinematter")
    *          )
    *     ),
    *    @OA\Response(
    *        response=200,
    *        description="OK",
    *        @OA\JsonContent(ref="#/components/schemas/Declinematter")
    *    ),
    *    security={{"api_key":{"PHPSESSID"}}}
    * )
    */
    ...
    {
        $model = new Declinematter();
        $model->setAttributes(Yii::$app->request->post());
	    $model->account_id = self::$user->account_id;
	    if (!$model->save()) {
            return self::error($model->getErrors());
        }
	    self::createLog('declinematter', serialize($model));
        return self::ok($model);
    }

    /**
    * @OA\Delete(
    *     path="/declinematter/delete",
    *     summary="Delete declinematter",
    *    description="delete  declinematter",
    *     tags={"Declinematter"},
    *     @OA\Parameter(
    *         description="Declinematter id to delete",
    *         in="query",
    *         name="id",
    *         required=true,
    *         @OA\Schema(
    ...
    */
    public function actionDelete()
	{
		...
		if (!$model) {
			self::error('Declinematter not found with id: ' . $id);
			return;
		}
	    if ($model->deleted_at) {
		...
		if ($model->update()) {
					self::deleteLog('declinematter', $id);
			return self::ok();
		} else {
...
/**
    * @OA\Get(
    *     path="/declinematter/restore",
    *     summary="Restore declinematter",
    *    description="restore  declinematter",
    *     tags={"Declinematter"},
    *     @OA\Parameter(
    *         description="Declinematter id to delete",
    *         in="query",
    ...
    */
    public function actionRestore()
	{
		...
		if ($model->update()()) {
			self::restoreLog('declinematter', $id);
			return self::ok();
		} else {
		    self::error($model->getErrors());
		}
    ...
    * @OA\Post(
    *    tags={"Declinematter"},
    *    path="/declinematter/update",
    *    summary="update  declinematter ",
    *    description="update declinematter",
    *    @OA\RequestBody(
    *        @OA\MediaType(
    *            mediaType="application/json",
    *            @OA\Schema(ref="#/components/schemas/Declinematter")
    ...
    */
    ...
    public function actionDownloadlist()
    {
		...
		return Excel::export([
            ...
            'fileName' => $table,
            'models' => $resarr,
            'columns' => ['id', 'name', 'account_id', 'deleted_at', ],
            'headers' => ['id'  => 'id', 'name'  => 'name', 'account_id'  => 'account_id', 'deleted_at'  => 'deleted_at', ],
        ]);
    }
    ...
    private function updateRecord($v){
        ...
        if ($v != null && is_array($v)) {
            ...
            if ($isset == self::RES_TRUE) {
                $model = Declinematter::findOne($v['id']);
                if ($model == null) {
                    $model = new Declinematter();
                }
				if($v['id']){
	...
	private function issetParams(array $array){
		$this->error = [];
		$nameArr = ['id', 'name', 'account_id', 'deleted_at', ];
	...
	public function actionUpload()
	{
        $model = new Upload();
		$id = Yii::$app->request->post('id');
		$owner = Declinematter::findOne($id);
