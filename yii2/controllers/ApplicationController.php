...
class ApplicationController extends BaseController
{
    ...
    */
    public function actionIndex()
    {
		...
		$q = new Query;
	 	$q->select('a.*, c.name, c.city_id, c.id as client_id');
		$q->from('application a');
		$q->leftJoin('clients c', 'a.clients_id = c.id'); 
		$q->andWhere(['a.account_id' => self::$user->account_id]);
	    $q->orderBy('a.id asc');
		...
		if ($userId) {
		    $q->andWhere(['a.manager_id' => $userId]);
	    }
		if ($dateFrom) {
		    $q->andWhere('a.startdate >= :dateFrom' , [ 'dateFrom' => date("Y-m-d H:i:s" , strtotime ($dateFrom) ) ]);
	    }
		if ($dateTo) {
		    $q->andWhere('a.startdate <= :dateTo' , ['dateTo' => date("Y-m-d H:i:s" , strtotime ($dateTo) ) ]);
	    }
		if ($query) {
			$q->andWhere('lower(a.comment) like :q ' , ['q' => $query ]); 
		}
        $cnt = $q->count();
    ...
    *            @OA\Schema(ref="#/components/schemas/Application")
    *          )
    *     ),
    *    @OA\Response(
    *        response=200,
    ...
    */
    public function actionCreate()
    {
		...
		$appModel->setAttributes(Yii::$app->request->post() );
		
        if($appModel->lead_id == null){
		    self::error('lead is not exsist!');
		}
		$leadModel = Lead::findOne($appModel->lead_id);
			
		$leadModel->enddate=date('Y-m-d H:i:s');
		$leadModel->state = Lead::STATUS_IN_BID;
		$appModel->clients_id = $leadModel->clients_id;
		$appModel->source_id = $leadModel->source_id;
		$appModel->startdate = date('Y-m-d H:i:s');
		$appModel->manager_id = self::$user->id;
		$appModel->account_id = self::$user->account_id;
		$appModel->contactdate=date('Y-m-d H:i:s',strtotime($appModel->contactdate));
		
		if (!$appModel->save()) {
			return self::error($appModel->getErrors());
		}
		if(!$leadModel->save()) {
			return self::error($leadModel->getErrors());
		}
		
		self::createLog('application', serialize($appModel));
		self::createLog('lead', serialize($leadModel));
		return self::ok($appModel);
	}	
    ...
    */
    public function actionDelete()
	{
		...
		if (!$model) {
			self::error('Application not found with id: ' . $id);
			return;
		}
				if ($model->deleted_at) {
...
/**
    ...
    */
    public function actionRestore()
	{
		...
		if ($model->update()()) {
			self::restoreLog('application', $id);
			return self::ok();
		} else {
		    self::error($model->getErrors());
		}
    ...
    */
    public function actionUpdate()
    {
		...
		$model->okp = $modelNew->okp; 
		$model->okpd2 = $modelNew->okpd2; 
		$model->tnved = $modelNew->tnved; 
		...
		$model->lead_id = $modelNew->lead_id; 
		...
		$model->field = $modelNew->field; 
		$model->countrymade = $modelNew->countrymade; 
		$model->countryask = $modelNew->countryask; 
		$model->comment = $modelNew->comment; 
		$model->exittoclient = $modelNew->exittoclient; 
		$model->test = $modelNew->test; 
		$model->total = $modelNew->total; 
		$model->our_cost = $modelNew->our_cost; 
		$model->documentprod = $modelNew->documentprod; 
		$model->showed = $modelNew->showed; 
		$model->countmanager_id = $modelNew->countmanager_id; 
    ...
    public function actionDownloadlist()
    {
		...
		return Excel::export([
            ...
            'fileName' => $table,
            'models' => $resarr,
            'columns' => ['id', 'clients_id', 'account_id', 'source_id', 'nameproduct', 'okp', 'okpd2', 'tnved', 'startdate', 'enddate', 'lead_id', 'manager_id', 'contactdate', 'state', 'field', 'countrymade', 'countryask', 'comment', 'exittoclient', 'test', 'total', 'our_cost', 'documentprod', 'showed', 'countmanager_id', 'declinematter_id', 'doc_on_hand', 'is_lpr', 'another_offer', 'doc_for_what', 'deleted_at', ],
            'headers' => ['id'  => 'id', 'clients_id'  => 'clients_id', 'account_id'  => 'account_id', 'source_id'  => 'source_id', 'nameproduct'  => 'nameproduct', 'okp'  => 'okp', 'okpd2'  => 'okpd2', 'tnved'  => 'tnved', 'startdate'  => 'startdate', 'enddate'  => 'enddate', 'lead_id'  => 'lead_id', 'manager_id'  => 'manager_id', 'contactdate'  => 'contactdate', 'state'  => 'state', 'field'  => 'field', 'countrymade'  => 'countrymade', 'countryask'  => 'countryask', 'comment'  => 'comment', 'exittoclient'  => 'exittoclient', 'test'  => 'test', 'total'  => 'total', 'our_cost'  => 'our_cost', 'documentprod'  => 'documentprod', 'showed'  => 'showed', 'countmanager_id'  => 'countmanager_id', 'declinematter_id'  => 'declinematter_id', 'doc_on_hand'  => 'doc_on_hand', 'is_lpr'  => 'is_lpr', 'another_offer'  => 'another_offer', 'doc_for_what'  => 'doc_for_what', 'deleted_at'  => 'deleted_at', ],
        ]);
    }
    ...
    private function updateRecord($v){
        ...
        if ($v != null && is_array($v)) {
            ...
            if ($isset == self::RES_TRUE) {
                $model = Application::findOne($v['id']);
                if ($model == null) {
	...
	private function issetParams(array $array){
		$this->error = [];
		$nameArr = ['id', 'clients_id', 'account_id', 'source_id', 'nameproduct', 'okp', 'okpd2', 'tnved', 'startdate', 'enddate', 'lead_id', 'manager_id', 'contactdate', 'state', 'field', 'countrymade', 'countryask', 'comment', 'exittoclient', 'test', 'total', 'our_cost', 'documentprod', 'showed', 'countmanager_id', 'declinematter_id', 'doc_on_hand', 'is_lpr', 'another_offer', 'doc_for_what', 'deleted_at', ];
