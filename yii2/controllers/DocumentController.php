...
class DocumentController extends BaseController
{
    ...
    */
    public function actionIndex()
    {
	    ...
	    $q = (new Query());
            $q->select('i.total,i.our_cost,i.nameproduct as prod_name, i.sert_num,i.document_id,
                         i.get_date,i.license_to,i.act_organ,
                         o.act_num, o.id,o.clients_id,o.manager_id,o.signact,o.signagree,
                         a.numberagree ,a.manager_id as a_man');
            $q->from('ork o, agreement a,item i');
            
            $q->where('o.agreement_id = a.id')
                    ->andWhere('i.application_id = a.id')
                    ->andWhere(['o.state'=>Ork::STATUS_ACCEPT]);
            $q->andWhere(['o.clients_id' => $clients_id]);  
			
        $q->andWhere(['o.account_id' => self::$user->account_id]);
		$q->andWhere(['i.account_id' => self::$user->account_id]);
...
//        return $q->createCommand()->getRawSql();
    ...
    * @OA\Post(
    *    tags={"Document"},
    *    path="/document/getAll",
    *    summary="getAll  document ",
    *    description="getAll",
    *    @OA\RequestBody(
    *        @OA\MediaType(
    ...
    */
	public function actionGetall(){
	    ...
	    $q = Document::find()->andWhere(['account_id' => self::$user->account_id]);
    ...
    */
	...
	self::createLog('document', serialize($model));
    ...
    */
    public function actionDelete()
	{
		...
		if (!$model) {
			self::error('Document not found with id: ' . $id);
			return;
		}
		if ($model->deleted_at) {
...
/**
    ...
    */
    public function actionUpdate()
    {
	    ...
	    $modelNew = new Document();
        $modelNew->setAttributes(Yii::$app->request->post());
		$model->name = $modelNew->name; 
		$model->fullname = $modelNew->fullname; 
		$model->for_doc = $modelNew->for_doc; 
		$model->is_include = $modelNew->is_include; 
		$model->is_control = $modelNew->is_control;         
		if ($model->save()) {
	        self::updateLog('document', $id);
            return self::ok($model);
        }
        return self::error($model->getErrors());
    }
    ...
    public function actionDownloadlist()
    {
		...
		return Excel::export([
            ...
            'fileName' => $table,
            'models' => $resarr,
            'columns' => ['id', 'name', 'account_id', 'fullname', 'for_doc', 'is_include', 'is_control', 'deleted_at', ],
            'headers' => ['id'  => 'id', 'name'  => 'name', 'account_id'  => 'account_id', 'fullname'  => 'fullname', 'for_doc'  => 'for_doc', 'is_include'  => 'is_include', 'is_control'  => 'is_control', 'deleted_at'  => 'deleted_at', ],
        ]);
    }
	...
	private function issetParams(array $array){
		$this->error = [];
		$nameArr = ['id', 'name', 'account_id', 'fullname', 'for_doc', 'is_include', 'is_control', 'deleted_at', ];
